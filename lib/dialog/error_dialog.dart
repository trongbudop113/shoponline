...
class ErrorDialog extends StatefulWidget {
  ...
  ErrorDialog({this.title, this.msg});
...
class _ErrorDialogState extends State<ErrorDialog> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final double itemHeight = (Common.isPortrait(context) ? size.height : size.width) * (widget.msg.length > 60 ? 0.35 : (Common.isPortrait(context) ? 0.28 : 0.33));
    print(widget.msg.length / 25);
